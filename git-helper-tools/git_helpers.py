...
def check_and_add_changes_before_commit():
    """Check for unstaged changes and ask the user if they want to add them before committing."""
    try:
        ...
        if unstaged_changes:
            print(Fore.YELLOW + "There are unstaged changes:")
            print(unstaged_changes)
            choice = input("Do you want to stage these changes before committing? (y/n): ").strip().lower()

            if choice in ["y", "yes"]:
                try:
                    subprocess.run(["git", "add", "-A"], check=True)
                    print(Fore.GREEN + "Changes staged successfully.")
                except subprocess.CalledProcessError:
                    print(Fore.RED + "Error: Unable to stage changes.")
                    return False
            elif choice in ["n", "no"]:
                print("Proceeding without staging changes.")
            else:
                print(Fore.RED + "Invalid input. Please enter 'y' or 'n'.")
                return check_and_add_changes_before_commit()
        else:
            print(Fore.GREEN + "No unstaged changes detected.")
        return True

    except subprocess.CalledProcessError:
        print(Fore.RED + "Error: Unable to check for unstaged changes.")
        return False
    ...
    while True:
        ...
        if push_choice in ['b', 'esc']:
            ...
            try:
                ...
                print("Error: Unable to push the changes. Your branch may be behind the remote. Use 'git pull' to sync changes.")
            return
...
def delete_commit_by_number(commits, commit_number):
    ...
    try:
        ...
        print("Error: Invalid commit number. Please ensure the number is within the range of available commits.")
    except subprocess.CalledProcessError:
        print("Error: Unable to delete commit. Ensure the commit number is correct and try again.")
        exit(1)
    ...
    while True:
        ...
        if action:
            try:
                ...
                log_action("Force pushed changes after commit deletion.")
            except subprocess.CalledProcessError:
                print("Error: Unable to force push the changes.")
                log_action("Force push failed after commit deletion.", status="ERROR")
                exit(1)
        break
...
def list_branches(show=False):
    """List all available branches. Optionally print them."""
    try:
...
def manage_branches():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print(Fore.CYAN + "Fetching latest branches from remote...")
            retry_operation(subprocess.run, ["git", "fetch"], check=True)
            branches = list_branches(show=True)
            branch_number = input("Enter the number of the branch to switch to: ").strip()
            if branch_number.lower() == 'b' or choice == "5":
                print("Returning to the main menu.")
                return
            try:
                branch_index = int(branch_number) - 1
                if branch_index < 0 or branch_index >= len(branches):
                    raise ValueError
                branch_name = branches[branch_index]
            except ValueError:
                print(Fore.RED + "Error: Invalid branch number.")
                continue
            retry_operation(subprocess.run, ["git", "checkout", branch_name], check=True)
            print(Fore.GREEN + f"Switched to branch '{branch_name}'.")
            print(Fore.CYAN + "Pulling latest changes for the branch...")
            quick_pull()
            ...
            if not branch_name.isalnum():
                print(Fore.RED + "Error: Branch name must be alphanumeric.")
                continue
...
def fetch_and_checkout_remote_branch():
    ...
    retry_operation(subprocess.run, ["git", "checkout", "-b", local_branch, branch_name], check=True)
...
def quick_push():
    ...
    try:
        ...
        log_action("Pushed changes to remote.")
    except subprocess.CalledProcessError:
        print(Fore.RED + "Error: Push failed.")
        log_action("Push failed.", status="ERROR")
...
def quick_pull():
    ...
    try:
        ...
        log_action("Pulled changes from remote.")
    except subprocess.CalledProcessError:
        print(Fore.RED + "Error: Pull failed.")
        log_action("Pull failed.", status="ERROR")
